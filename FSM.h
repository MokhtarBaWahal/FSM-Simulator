...
public:
    ...
    const std::string &getStartStateName() const {
        return startStateName;
    }
    void setCurrentInput(int input) {
        currentInput = input;
    }
    void setRunningFlag(bool input) {
        running = input;
    }
    ...
    bool canTransitionTo(const std::string& nextStateName) {
        std::string validState = transitions.getTransition(currentState->getName(), currentInput);
        return !validState.empty() && validState == nextStateName;
    }

    void changeStateDirectly(const std::string& nextStateName) {
        for (size_t i = 0; i < states.size(); i++) {
            if (states[i]->getName() == nextStateName && canTransitionTo(nextStateName)) {
